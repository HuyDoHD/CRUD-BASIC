...
import { closeInMemoryMongo, connectInMemoryMongo } from '../../../../test/setup';
import { EventDocument, Events, EventSchema } from '../../../schemas/event.schema';
...
describe('EventService', () => {
  ...
  beforeAll(async () => {
    ...
    eventModel = mongoose.model<Events>('Events', EventSchema) as Model<EventDocument>;
  ...
  it('should release editing lock', async () => {
    await service.releaseEdit('64f06cfa8f4c3b2b1a2e1f33', { userId: 'user1', email: 'user1', role: 'admin' });
  ...
  it('should maintain editing lock', async () => {
    ...
    await service.maintainEdit('64f06cfa8f4c3b2b1a2e1f33', { userId: 'user1', email: 'user1', role: 'admin' });
  ...
  it('should throw if another user is editing and lock not expired', async () => {
    ...
    await expect(
      service.requestEdit('64f06cfa8f4c3b2b1a2e1f33', { userId: 'user1', email: 'user1', role: 'admin' })
  ...
  it('should throw if editing non-existent event', async () => {
    await expect(
      service.editEvent('000000000000000000000000', { name: 'Invalid Event' }, {
        userId: 'user1',
        email: 'user1',
        role: 'admin',
      })
